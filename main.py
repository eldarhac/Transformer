from __future__ import annotations
import torch

STUDENT={'name': 'Eldar Hacohen',
         'ID': '311587661'}

if __name__ == '__main__':
    import torch
    from torch import nn
    from torch import optim
    from transformer import TransformerLM
    import data
    import lm

    path_to_model = "model_shakespeare.pth"

    seq_len = 128
    batch_size = 64
    data_path = "data/"
    n_layers = 8
    n_heads = 8
    embed_size = 192
    mlp_hidden_size = embed_size * 4

    learning_rate = 5e-4
    gradient_clipping = 1.0

    num_batches_to_train = 50000

    tokenizer, tokenized_data = data.load_data(data_path)
    # NOTE: are data items are longer by one than the sequence length,
    # They will be shortened by 1 when converted to training examples.
    data_iter = iter(data.RandomOrderDataIterator(tokenized_data, seq_len + 1))

    model: torch.nn.Module = TransformerLM(
            n_layers,
            n_heads,
            embed_size,
            seq_len,
            tokenizer.vocab_size(),
            mlp_hidden_size,
            with_residuals = True,
        )

    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, betas=[0.9, 0.95])

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model.to(device)

    model.train()

    num_batches = 0
    for batch in data.batch_items(data_iter, batch_size):

        batch_x, batch_y = lm.batch_to_labeled_samples(batch)

        batch_x = batch_x.to(device)
        batch_y = batch_y.to(device)

        logits = model(batch_x)

        loss = lm.compute_loss(logits, batch_y)

        # parameters update
        model.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), gradient_clipping)
        optimizer.step()

        num_batches += 1
        if num_batches % 10 == 0:
            print(f"Seen {num_batches} batches. last loss is: {loss.item()}")
            if num_batches % 100 == 0:
                for _ in range(1):
                    model.eval()
                    sampled = tokenizer.detokenize(model.sample_continuation(tokenizer.tokenize("Hello"), 500))
                    # sampled = tokenizer.detokenize(model.better_sample_continuation(tokenizer.tokenize("Hello"),
                    #                                                                 500, temperature=0.5, topK=5))
                    model.train()
                    print(f"Model sample: '''{sampled}'''")
                print("")
        if num_batches >= num_batches_to_train:
            break


    print(f"Training finished. loss: {loss.item()} saving model to {path_to_model}")
    torch.save(model.state_dict(), path_to_model)
